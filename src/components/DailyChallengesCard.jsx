...
const DailyChallengesCard = ({ challenges = [], rewards = { coins: 50, xp: 20 } }) => {
    ...
    <div className="col-span-2 bg-gradient-to-b from-[#18181b] to-[#09090b] p-6 rounded-[2.5rem] border border-white/10 relative overflow-hidden shadow-lg">
      ...
      <div className="relative z-10 flex flex-col gap-5">
        ...
        <div className="h-2 w-full bg-black/60 rounded-full overflow-hidden border border-white/5 p-[1px] shadow-inner">
          <div 
            className="h-full bg-gradient-to-r from-emerald-500 to-emerald-300 rounded-full relative transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(16,185,129,0.5)]"
          ...
          {challenges.map((challenge, idx) => (
            ...
            >
              ...
              <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 ${
                ...
                {challenge.completed && <span className="text-[10px] font-black">✓</span>}
              </div>
              
              {/* Task Title */}
              <div className="flex-1">
                <p className={`text-sm font-bold transition-colors ${
                  challenge.completed ? 'text-white/40 line-through' : 'text-white/90'
                }`}>
                  {challenge.title}
                </p>
              </div>
