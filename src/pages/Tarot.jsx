...
const Tarot = () => {
  ...
  return (
    <div className="flex flex-col w-full pb-10 animate-fade-in kundali-grid min-h-screen relative overflow-hidden">
      ...
      <div className="px-6 pt-8 pb-4 text-center space-y-2 relative z-10">
        <h1 className="text-3xl font-black premium-gradient-text tracking-tight uppercase text-white">{t.title}</h1>
        <p className="text-white/40 text-[11px] max-w-[280px] mx-auto font-medium italic leading-relaxed">
          {t.subtitle}
        </p>
      </div>

      {!showResult ? (
        <div className="px-4 space-y-8 relative z-10">
          <p className="text-[10px] text-mystic-gold uppercase tracking-[0.2em] font-black text-center mb-0 opacity-80">
            {t.instruction}
          </p>

          <div className="flex justify-center items-center gap-4 h-[200px] perspective-1000 relative">
            {[0, 1, 2].map((i) => {
              ...
              let moveTransform = '';
              if (isSelected) {
                 if (i === 0) moveTransform = 'translateX(116px)';
                 if (i === 2) moveTransform = 'translateX(-116px)';
              }

              const finalTransform = moveTransform + (isFlipping && isSelected ? ' rotateY(180deg) scale(1.4)' : '');
              ...
              return (
                <div 
                  ...
                  className={`relative w-[100px] h-[170px] transition-all duration-700 preserve-3d cursor-pointer 
                ...
                >
                  ...
                  <div className="absolute inset-0 w-full h-full preserve-3d">
                    <div className="absolute inset-0 backface-hidden glass rounded-xl border-2 border-mystic-gold/30 flex flex-col items-center justify-center overflow-hidden z-20 transition-colors duration-500">
                       ...
                       <div className="w-20 h-32 border border-mystic-gold/5 rounded-md flex items-center justify-center bg-gradient-to-br from-mystic-purple/20 to-transparent">
                          <span className={`text-4xl transition-all duration-500 ${hoveredIndex === i && !selectedCard ? 'opacity-100 scale-110' : 'opacity-30'}`}>🕉️</span>
                       </div>
                    </div>
                    ...
                    <div className="absolute inset-0 backface-hidden rotate-y-180 glass rounded-xl border-2 border-mystic-gold flex flex-col items-center justify-center z-10 bg-mystic-indigo/90 text-white">
                       {selectedCard?.image ? (
                          <div className="flex flex-col items-center space-y-2">
                             ...
                             <span className="text-[10px] font-black text-mystic-gold uppercase tracking-widest">{selectedCard.name}</span>
                          </div>
                       ) : (
          ...
          {history.length > 0 && !selectedCard && (
            <div className="space-y-4 pt-2">
               <div className="flex items-center gap-3">
                  <h3 className="text-[9px] uppercase tracking-[0.3em] font-black text-white/30">{t.pastRevelations}</h3>
               ...
               <div className="grid grid-cols-1 gap-2">
                  {history.slice(0, 3).map((h, i) => (
                    <div key={i} className="glass px-4 py-3 rounded-2xl border border-white/5 flex justify-between items-center group hover:border-mystic-gold/20 transition-all">
                       <span className="text-xs font-bold text-white/70">{h.cardName}</span>
        ...
        <div className="px-6 animate-fade-in pb-12 relative z-10">
           <div className="glass-card p-6 rounded-[40px] border border-mystic-gold/20 space-y-6 relative overflow-hidden">
              ...
              <div className="flex flex-col items-center text-center space-y-4">
                 <div className="w-36 h-56 glass rounded-2xl border-2 border-mystic-gold shadow-[0_0_50px_rgba(251,191,36,0.25)] flex flex-col items-center justify-center space-y-4 relative bg-mystic-indigo/40 text-white">
                    <span className="text-7xl drop-shadow-[0_0_20px_rgba(251,191,36,0.6)]">{selectedCard?.image}</span>
                    <h2 className="text-lg font-black text-mystic-gold uppercase tracking-[0.1em]">{selectedCard?.name}</h2>
                 </div>
              </div>
      ...
      <div className="mt-8 px-10 relative z-10">
         <button 
           onClick={() => navigate('/')} 
           className="w-full py-4 glass-card rounded-2xl border border-white/10 text-[10px] font-black text-white/40 uppercase tracking-[0.4em] hover:text-white hover:border-mystic-gold/30 hover:bg-white/5 transition-all shadow-xl"
         >
            {t.backToTemple}
         </button>
      </div>
