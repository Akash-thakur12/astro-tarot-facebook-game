export function safeParseMemoryState(rawText) {
  ...
  if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
    ...
    // rejectedPaths
    if (parsed.rejectedPaths !== undefined) {
      if (Array.isArray(parsed.rejectedPaths)) {
        const validPaths = parsed.rejectedPaths.filter(item => item && typeof item.path === 'string' && typeof item.cooldown === 'number');
        if (validPaths.length !== parsed.rejectedPaths.length) schemaFixed = true;
        result.memoryState.recommendationMemory.rejectedPaths = validPaths;
      } else {
        schemaFixed = true;
      }
    }
...
export function mergeRecommendationMemory(existingMemory, newMemory) {
  ...
  const merged = {
    ...
    rejectedPaths: [...(newMemory.rejectedPaths || existingMemory.rejectedPaths || [])]
  };
