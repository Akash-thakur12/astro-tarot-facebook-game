...
import { generateAIResponse as generateBase, generateAIResponseStream } from '../services/aiService.js';
...
export function extractAndRemoveSecrets(text) {
...
export function processRawResponse(rawText, options) {
  let aiText = rawText;
...
export async function executeAIWithRetries(options) {
  ...
  if (mode === 'chat' || mode === 'personal') {
    ...
    const userQuestionForPrompt = userData?.question || "";
    const boundaryYear = extractBoundaryYear(userQuestionForPrompt, pastHistory);
    
    // Calculate transit years programmatically
    const jupiter5th = calculateTransitYears(astroData, 5, 'jupiter');
    const jupiter7th = calculateTransitYears(astroData, 7, 'jupiter');
    const jupiter9th = calculateTransitYears(astroData, 9, 'jupiter');
    const jupiter10th = calculateTransitYears(astroData, 10, 'jupiter');
    const jupiter11th = calculateTransitYears(astroData, 11, 'jupiter');
    
    const saturn1st = calculateTransitYears(astroData, 1, 'saturn');
    const saturn7th = calculateTransitYears(astroData, 7, 'saturn');
    const saturn10th = calculateTransitYears(astroData, 10, 'saturn');

    const birthYear = userData?.dobYear || null;
    const userAgeIn2026 = birthYear ? (2026 - parseInt(birthYear)) : 'Unknown';
...
- Active Major Planet End (Mahadasha End): ${astroData?.mahadashaEnd || 'Unknown'}
- Active Sub Planet (Antardasha): ${astroData?.antardasha || 'Unknown'}
- Active Sub Planet End (Antardasha End): ${astroData?.antardashaEnd || 'Unknown'}
- Target Houses for Query: ${detectedHouses}
- Current Calendar Year: 2026
- User Birth Year: ${birthYear || 'Unknown'}
- User Age in 2026: ${userAgeIn2026}
---------------------------------

--- OPTIONAL SUPPORTING PLANETARY TRANSIT WINDOWS (WEAK SIGNALS ONLY) ---
- Jupiter (Guru) transit years (expansion periods):
  - 5th House (Love/Children): ${jupiter5th.join(', ')}
  - 7th House (Marriage/Partnership): ${jupiter7th.join(', ')}
  - 9th House (Fortune/Long Travel): ${jupiter9th.join(', ')}
  - 10th House (Career/Status): ${jupiter10th.join(', ')}
  - 11th House (Income/Gains): ${jupiter11th.join(', ')}
- Saturn (Shani) transit years (delay/structure periods):
  - 1st House (Self/Health): ${saturn1st.join(', ')}
  - 7th House (Marriage lessons/delay): ${saturn7th.join(', ')}
  - 10th House (Career structure/duty): ${saturn10th.join(', ')}
------------------------------------------------------------------------

--- USER-SPECIFIED TIMING BOUNDARIES & CONSTRAINTS ---
- Boundary constraint: ${boundaryYear ? `Events must ONLY be predicted in/after the year ${boundaryYear}` : 'None specified'}
-------------------------------------------------------
...
- STATED USER REJECTIONS / COOLDOWNS (Do NOT suggest these): [${(recommendationMemory.rejectedPaths || []).map(p => p.path).join(', ') || "None yet"}]
...
`;
  ...
  console.log("=== DEBUG TIMING INFO ===");
  console.log("active_mahadasha:", astroData?.mahadasha || 'Unknown');
  console.log("active_antardasha:", astroData?.antardasha || 'Unknown');
  console.log("mahadashaEnd:", astroData?.mahadashaEnd || 'Unknown');
  console.log("antardashaEnd:", astroData?.antardashaEnd || 'Unknown');
  console.log("=========================");

  console.log("PROMPT_SIZE:", injectedPrompt.length);
  console.log("PROMPT_SIZE_CHARS:", injectedPrompt.length);
  console.log("HISTORY_MESSAGES_COUNT:", (history || []).length);
  const astroDataSize = astroData ? JSON.stringify(astroData).length : 0;
  console.log("ASTRO_MATRIX_SIZE:", astroDataSize);
  const totalContextSize = injectedPrompt.length + (history || []).map(m => (m.content || '').length).reduce((a, b) => a + b, 0);
  console.log("TOTAL_CONTEXT_SIZE:", totalContextSize);
  const prepStartTime = options.prepStartTime || Date.now();
  console.log("TOTAL_PREP_TIME:", (Date.now() - prepStartTime) + "ms");
  ...
  const userQuestion = userData?.question || "";
  const boundaryYear = extractBoundaryYear(userQuestion, pastHistory);
  ...
  const isQualityValid = validateConversationalQuality(aiText, pastHistory, userQuestion, userData, astroData, boundaryYear);

  needsRetry = !isQualityValid ||
               containsForbiddenPhrases(aiText, updatedFacts) || 
               !validateAstroResponse(validatedRetryText, astroData, skipDashaPreservation) ||
               containsRepetitiveCliffhanger(aiText, pastHistory) ||
               failsRevealRequest(aiText, userQuestion);

  if (needsRetry) {
    if (!isQualityValid) {
      if (aiText.toLowerCase().match(/\b(2027|2028|2029)\b/) && (boundaryYear && boundaryYear >= 2030)) {
        retryReason = "year_spam";
      } else {
        const roboticOpenings = [
          /suno\s+\w+/i,
          /tumhari\s+\w+\s+aur\s+\w+\s+rashi/i,
          /tumhari\s+lagna/i,
          /tumhara\s+lagna/i,
          /tumhari\s+rashi/i,
          /tumhara\s+rashi/i,
          /namaste\s+beta/i,
          /pranam\s+beta/i,
          /kalyan\s+ho\s+beta/i
        ];
        if (roboticOpenings.some(pattern => pattern.test(aiText))) {
          retryReason = "robotic_opening";
        } else {
          retryReason = "quality_failed";
        }
      }
    } else if (containsForbiddenPhrases(aiText, updatedFacts)) {
      retryReason = "blacklist";
    } else if (containsRepetitiveCliffhanger(aiText, pastHistory)) {
      retryReason = "repetitive_cliffhanger";
    } else if (failsRevealRequest(aiText, userQuestion)) {
      retryReason = "fails_reveal";
    } else {
  ...
  while (needsRetry && retryCount < 2) {
    ...
    if (retryReason === "year_spam") {
      retryPrompt += `\n\nSYSTEM WARNING: YEAR_SPAM_DETECTED. You predicted a year (2027/2028/2029) that violates the user's boundary constraint (events must be after/in ${boundaryYear}). Use transit windows only as weak supporting signals. Primary timing authority remains: 1) Dasha, 2) Antardasha, 3) Existing astrology engine, 4) User constraints, 5) Conversation history. If timing confidence is low, express uncertainty naturally.`;
    } else if (retryReason === "robotic_opening") {
      retryPrompt += `\n\nSYSTEM WARNING: ROBOTIC_OPENING_DETECTED. Do NOT start with robotic templates like "Suno [Name]" or "Tumhari [Lagna]/[Rashi]". Blend insights naturally. Talk like a real WhatsApp Pandit or friend.`;
    } else if (retryReason === "quality_failed") {
      retryPrompt += `\n\nSYSTEM WARNING: CONVERSATIONAL_QUALITY_FAILED. Your response failed quality guidelines. Talk like a wise friend, use conversational Hinglish ("Dekho...", "Ek baat bataun..."), move the story forward, and do not repeat previous astrology setups or facts.`;
    } else if (retryReason === "blacklist") {
    ...
    } else if (retryReason === "repetitive_cliffhanger") {
      retryPrompt += `\n\nSYSTEM WARNING: Do not repeat previous cliffhanger phrases like "kal subah bataunga" or "24 ghante me". Rotate your hooks creatively.`;
    } else if (retryReason === "fails_reveal") {
      retryPrompt += `\n\nSYSTEM WARNING: The user asked a direct question. You MUST reveal the answer immediately (e.g. Ketu, Saturn) with zero hesitation. Do NOT stall or say "kal bataunga".`;
    } else {
      retryPrompt += `\n\n[SYSTEM WARNING: Please generate a new response. Answer differently and avoid repeating previous wording.]`;
    }

    console.log("=== DEBUG TIMING INFO (RETRY) ===");
    console.log("active_mahadasha:", astroData?.mahadasha || 'Unknown');
    console.log("active_antardasha:", astroData?.antardasha || 'Unknown');
    console.log("mahadashaEnd:", astroData?.mahadashaEnd || 'Unknown');
    console.log("antardashaEnd:", astroData?.antardashaEnd || 'Unknown');
    ...
    const isQualityValid2 = validateConversationalQuality(aiText, pastHistory, userQuestion, userData, astroData, boundaryYear);
    needsRetry = !isQualityValid2 ||
                 containsForbiddenPhrases(aiText, updatedFacts) || 
                 !validateAstroResponse(validatedRetryText2, astroData, skipDashaPreservation) ||
                 containsRepetitiveCliffhanger(aiText, pastHistory) ||
                 failsRevealRequest(aiText, userQuestion);
    ...
    if (needsRetry) {
      if (!isQualityValid2) {
        ...
        let dashaMissing = false;
        if (astroData && !skipDashaPreservation) {
          const lower = validatedRetryText2.toLowerCase();
          if (astroData.mahadasha && !lower.includes(astroData.mahadasha.toLowerCase())) dashaMissing = true;
          if (astroData.antardasha && !lower.includes(astroData.antardasha.toLowerCase())) dashaMissing = true;
        }
        retryReason = dashaMissing ? "dasha_missing" : "hallucination";
...
export async function executeAIWithRetriesStream(options) {
  const {
    fullPrompt, history, astroData, mode, uid, userData, progress,
    detectedIntent, pastHistory, skipDashaPreservation, resolvedLanguage,
    isDevanagari, maritalStatus, updatedFacts, isGreeting, isVague
...
`;
  ...
  console.log("=== DEBUG TIMING INFO (STREAM) ===");
  console.log("active_mahadasha:", astroData?.mahadasha || 'Unknown');
  console.log("active_antardasha:", astroData?.antardasha || 'Unknown');
  console.log("mahadashaEnd:", astroData?.mahadashaEnd || 'Unknown');
  console.log("antardashaEnd:", astroData?.antardashaEnd || 'Unknown');
...
export async function injectSecretAndScore(text, uid, userData, cachedProgress = null, category = 'General', pastHistory = [], llmSecret = "") {
  if (!text) return text;
...
function containsRepetitiveCliffhanger(text, pastHistory) {
  if (!text || !pastHistory || pastHistory.length === 0) return false;
  const lowerText = text.toLowerCase();
  const repetitivePhrases = ["kal subah bataunga", "24 ghante me raaz khulega", "gupt grah", "kal bataunga", "raaz khulega", "agle 24 ghante"];
  const lastAssistantMsgs = pastHistory.filter(m => m.role === 'model' || m.role === 'assistant').slice(-3);
  for (const phrase of repetitivePhrases) {
    if (lowerText.includes(phrase)) {
      for (const msg of lastAssistantMsgs) {
        if (msg.content && msg.content.toLowerCase().includes(phrase)) {
          console.log("REPETITIVE CLIFFHANGER DETECTED:", phrase);
          return true;
        }
      }
    }
...
function isDirectRevealRequest(queryText) {
  if (!queryText) return false;
  const lower = queryText.toLowerCase();
  return (
    lower.includes("kaun sa") || 
    lower.includes("kon sa") || 
    lower.includes("kaunsa") || 
    lower.includes("konsa") || 
    lower.includes("naam batao") || 
    lower.includes("kya naam") || 
    lower.includes("kab thik") || 
    lower.includes("kab hoga") || 
    lower.includes("kab tak") ||
    lower.includes("kaise") ||
    lower.includes("kis field") ||
    lower.includes("kaunsi dasha") ||
    lower.includes("dasha kon") ||
    lower.includes("konsi dasha") ||
    lower.includes("kaunsa grah") ||
    lower.includes("kon sa grah")
  );
}

function failsRevealRequest(text, queryText) {
  if (!isDirectRevealRequest(queryText)) return false;
  console.log("[REVEAL_PRIORITY_TRIGGERED] User direct query:", queryText);
  const lowerText = text.toLowerCase();
  const stallingPhrases = ["kal bataunga", "kal subah", "suspense", "raaz khulega", "baad me bataunga", "intazar karein"];
  for (const phrase of stallingPhrases) {
    if (lowerText.includes(phrase)) {
      console.log("FAIL REVEAL REQUEST: Stalling phrase used:", phrase);
      return true;
    }
  }
  return false;
...
export function extractBoundaryYear(queryText, pastHistory) {
  const lowerQuery = (queryText || "").toLowerCase();
  let boundaryYear = null;
  const boundaryMatch = lowerQuery.match(/(?:until|before|till|baad|se pehle)\s*(\d{4})/i) ||
                        lowerQuery.match(/(\d{4})\s*(?:se pehle|tak nahi|ke baad)/i) ||
                        lowerQuery.match(/(?:not|won't|don't).*?(\d{4})/i);
  if (boundaryMatch) {
    boundaryYear = parseInt(boundaryMatch[1]);
  }
  
  if (!boundaryYear && pastHistory) {
    for (const msg of pastHistory) {
      if (msg.role === 'user' && msg.content) {
        const hQuery = msg.content.toLowerCase();
        const hMatch = hQuery.match(/(?:until|before|till|baad|se pehle)\s*(\d{4})/i) ||
                       hQuery.match(/(\d{4})\s*(?:se pehle|tak nahi|ke baad)/i) ||
                       hQuery.match(/(?:not|won't|don't).*?(\d{4})/i);
        if (hMatch) {
          boundaryYear = parseInt(hMatch[1]);
          break;
        }
      }
    }
...
export function calculateTransitYears(astroData, targetHouse, type = 'jupiter') {
  const SIGNS = {
    mesh: 1, vrishabh: 2, mithun: 3, kark: 4, simha: 5, kanya: 6,
    tula: 7, vrishchik: 8, dhanu: 9, makar: 10, kumbh: 11, meen: 12
  };
  
  const lagnaStr = (astroData?.lagna || 'Mesh').toLowerCase();
  let lagnaSignNum = 1;
  for (const [sign, num] of Object.entries(SIGNS)) {
    if (lagnaStr.includes(sign)) {
      lagnaSignNum = num;
      break;
    }
  }
  ...
  let planetSignStr = '';
  if (type === 'saturn') {
    planetSignStr = (planets.Saturn || planets.shani || 'Kumbh').toLowerCase();
  } else {
    planetSignStr = (planets.Jupiter || planets.guru || 'Meen').toLowerCase();
  }
  
  let planetSignNum = 1;
  for (const [sign, num] of Object.entries(SIGNS)) {
    if (planetSignStr.includes(sign)) {
      planetSignNum = num;
      break;
    }
  }
  
  const currentHouse = (planetSignNum - lagnaSignNum + 12) % 12 + 1;
  
  const years = [];
  if (type === 'saturn') {
    const diffHouses = (targetHouse - currentHouse + 12) % 12;
    const baseYears = diffHouses * 2.5;
    for (let cycle = 0; cycle < 3; cycle++) {
      const startYear = Math.floor(2026 + baseYears + cycle * 30);
      const endYear = Math.floor(2026 + baseYears + 2.5 + cycle * 30);
      years.push(`${startYear}-${endYear}`);
    }
  } else {
    const diffHouses = (targetHouse - currentHouse + 12) % 12;
    const baseYears = diffHouses;
    for (let cycle = 0; cycle < 4; cycle++) {
      years.push(2026 + baseYears + cycle * 12);
    }
  }
  return years;
}

export function validateConversationalQuality(text, pastHistory, queryText, userData, astroData, boundaryYear) {
  if (!text) return true;
  const lowerText = text.toLowerCase();
  const lowerQuery = (queryText || "").toLowerCase();

  // 1. REVEAL PRIORITY TRIGGERED
  const isRevealReq = isDirectRevealRequest(lowerQuery);
  if (isRevealReq) {
    console.log("[REVEAL_PRIORITY_TRIGGERED] User asked a direct question:", queryText);
  }

  // 2. TEMPORAL_REASONING_CHECK
  console.log("[TEMPORAL_REASONING_CHECK] Checking time & year consistency");
  
  // Extract all years from the response text
  const yearRegex = /\b(2\d{3})\b/g;
  let match;
  const predictedYears = [];
  while ((match = yearRegex.exec(text)) !== null) {
    predictedYears.push(parseInt(match[1]));
  }

  if (boundaryYear) {
    for (const yr of predictedYears) {
      if (yr < boundaryYear) {
        console.log(`[YEAR_SPAM_DETECTED] Rejected: Predicted year ${yr} violates boundary constraint of ${boundaryYear}`);
        return false;
      }
    }
  }

  // Check for 2027/2028/2029 placeholder spam loops
  const hasFixedYears = predictedYears.some(yr => yr === 2027 || yr === 2028 || yr === 2029);
  if (hasFixedYears) {
    if (boundaryYear && boundaryYear >= 2030) {
      console.log("[YEAR_SPAM_DETECTED] Rejected: Contains 2027/2028/2029 but boundary is >= 2030");
      return false;
    }

    if (pastHistory) {
      const lastAssistantMsgs = pastHistory.filter(m => m.role === 'model' || m.role === 'assistant').slice(-2);
      for (const msg of lastAssistantMsgs) {
        if (msg.content) {
          const prevYears = msg.content.match(/\b(2027|2028|2029)\b/g);
          if (prevYears && prevYears.length > 0) {
            console.log("[YEAR_SPAM_DETECTED] Rejected: Repeating 2027/2028/2029 in consecutive turns");
            return false;
          }
        }
      }
  ...
  // 3. FORWARD_PROGRESS_CHECK
  console.log("[FORWARD_PROGRESS_CHECK] Verifying progression of conversation");

  // A. Robotic openings ban
  const roboticOpenings = [
    /suno\s+\w+/i,
    /tumhari\s+\w+\s+aur\s+\w+\s+rashi/i,
    /tumhari\s+lagna/i,
  ...
  for (const pattern of roboticOpenings) {
    if (pattern.test(text)) {
      console.log("[FORWARD_PROGRESS_CHECK] Rejected: Robotic opening detected:", pattern);
      return false;
    }
  }

  // B. Repeated astrology setup
  if (pastHistory) {
    const lastAssistant = [...pastHistory].reverse().find(m => m.role === 'model' || m.role === 'assistant');
    if (lastAssistant && lastAssistant.content) {
      const lagnaMentioned = lastAssistant.content.toLowerCase().includes("lagna");
      const rashiMentioned = lastAssistant.content.toLowerCase().includes("rashi");
      if (lagnaMentioned && rashiMentioned && (lowerText.includes("lagna") || lowerText.includes("rashi"))) {
        if (!lowerQuery.includes("lagna") && !lowerQuery.includes("rashi")) {
          console.log("[FORWARD_PROGRESS_CHECK] Rejected: Repeated lagna/rashi setup without user query request");
          return false;
        }
      }
    }
  ...
  // C. Previously revealed facts presented as new discoveries
  const discoveryPhrases = ["ek raaz", "gupt baat", "naya sanket", "reveal", "raaz khulega", "kal subah"];
  const hasDiscoveryPhrase = discoveryPhrases.some(phrase => lowerText.includes(phrase));
  if (hasDiscoveryPhrase && pastHistory) {
    const lastAssistantMsgs = pastHistory.filter(m => m.role === 'model' || m.role === 'assistant').slice(-3);
    for (const msg of lastAssistantMsgs) {
      if (msg.content) {
        const similarity = getJaccardSimilarity(text, msg.content);
        if (similarity > 0.5) {
          console.log("[FORWARD_PROGRESS_CHECK] Rejected: High similarity to past responses representing repeated info");
          return false;
        }
      }
    }
